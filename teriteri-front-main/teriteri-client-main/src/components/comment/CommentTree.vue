<template>
    <div class="reply-list">
        <div class="reply-item" v-for="(rootComment, index) in (userStore.isLogin ? commentList : commentList.slice(0,2))" :key="index">
            <!-- 第二楼特有的未登录蒙版 -->
            <div class="login-limit-mask" v-if="index === 1" :style="userStore.isLogin ? '' : 'display: block;'">
                                ...
                                <div class="reply-operation">
                                    ...
                                    <ul class="operation-list" :style="{ display: showOption ? '' : 'none' }">
                                        <li class="operation-option"
                                            v-if="userStore.user?.uid === rootComment.user.uid || userStore.user?.uid === upUid"
            ...
            <div class="reply-box-container" :class="[commentInfo.root_id === rootComment.id ? '' : 'hide']">
                <div class="reply-box box-active">
                    <ReplyTextarea ref="replyTextareas" :placeholder="replyPlaceHolder" :commentInfo="commentInfo" :isWideWindow="isWideWindow" @addComment="addComment"></ReplyTextarea>
                </div>
            </div>
        ...
        <div class="no-more" v-show="(userStore.isLogin && !hasMore && commentList.length !== 0) || (!userStore.isLogin && commentList.length === 1)">已经到底啦~</div>
        <div class="login-prompt" v-show="!userStore.isLogin && commentList.length >= 2" @click="toLogin">登录后查看更多评论</div>
    </div>
</template>
...
import { handleLevel, handleDateTime3, emojiText } from '@/utils/utils';
import { useUserStore } from '@/stores';
import {
    getCommentTree as getCommentTreeApi,
    deleteComment as deleteCommentApi,
    getUpLike as getUpLikeApi,
...
const commentList = ref<any[]>([]);     // 当前视频的所有回复
const hasMore = ref(true);              // 是否还有更多评论树
const loading = ref(false);             // 是否正在加载评论
const upLike = ref<number[]>([]);       // UP主觉得很赞
const replyPlaceHolder = ref("");       // 控制 回复@用户名：
const showOption = ref(false);          // 当前点击的哪条评论的展开操作栏按钮
const replyTextareas = ref<any[]>([]);  // v-for 中各个回复输入框的实例
// 回复评论的基础信息
...
const hotCommentIdx = ref(0);   // 热评id

// 获取评论树
async function getCommentTree() {
    if (!hasMore.value || loading.value || (!userStore.isLogin && commentList.value.length > 0)) return;
    loading.value = true;
    const res = await getCommentTreeApi({
        vid: Number(route.params.vid),
        offset: commentList.value.length,
        type: props.type,
    });
    if (!res.data) {
        loading.value = false;
...
// 删除评论前的最后通牒
function beforeDelete(id: number) {
...
async function deleteComment(cid: number) {
    ...
    commentList.value = commentList.value.filter((comment: any) => comment.id !== cid)
    ElMessage.info("删除成功");
}

// 删除子评论
function delSubComment(comment: any) {
    const rootComment = commentList.value.find((item: any) => item.id === comment.rootId);
    rootComment.replies = rootComment.replies.filter((cmt: any) => cmt.id !== comment.id);
}

// 获取UP主觉得很赞
async function getUpLike() {
    const res = await getUpLikeApi(props.upUid);
    if (!res.data) return;
...
function addComment(comment: any) {
    if (comment.rootId === 0) {
        ...
        // 回复评论尾插
        const root = commentList.value.find((item: any) => item.id === comment.rootId);
        if (root) root.replies.push(comment);
    }
}

// 点赞或点踩某条评论
async function likeOrDislike(id: number, like: boolean, set: boolean) {
    if (!userStore.user?.uid) {
        ElMessage.error("请登入");
        return;
    }

    await likeOrDislikeComment({ id: Number(id), isLike: like, isSet: set });

    // 更新store中的列表
    let likeExist = isLike(id);
    let dislikeExist = isDislike(id);
    let comment = commentList.value.find((comment: any) => comment.id === id)
...
function isLike(id: number) {
    let i = userStore.likeComment.findIndex((item: number) => item === id);
    return i !== -1;
}
// 判断用户是否对当前评论点踩
function isDislike(id: number) {
    let i = userStore.dislikeComment.findIndex((item: number) => item === id);
    return i !== -1;
}
// 取消点赞
function subLikeComment(id: number) {
    let lc = userStore.likeComment.filter((item: number) => item !== id)
    userStore.likeComment = lc;
}
// 点赞
function addLikeComment(id: number) {
    userStore.likeComment.push(id);
}
// 取消点踩
function subDislikeComment(id: number) {
    let dlc = userStore.dislikeComment.filter((item: number) => item !== id);
    userStore.dislikeComment = dlc;
}
// 点踩
function addDislikeComment(id: number) {
    userStore.dislikeComment.push(id);
}

// 更新获取到的全部子评论
function updateComment(comment: any) {
    const i = commentList.value.findIndex((item: any) => item.id === comment.id);
    if (i !== -1) {
        commentList.value[i].replies = comment.replies;
    }
}

// 控制回复框的聚焦事件
function cancelFocus() {
    replyTextareas.value.forEach((el: any) => {
        el?.cancelFocus();
    })
}
...
async function handleScroll() {
    ...
    if (commentBottom - windowHeight <= 300) await getCommentTree();
}

// 处理评论的commentInfo
function handleReply(comment: any) {
    if (comment.id === commentInfo.value.parent_id) {
        commentInfo.value = {
            root_id: -1,
            parent_id: -1,
            to_user_id: -1,
    ...
    replyPlaceHolder.value = "回复@ " + comment.user.nickname

    // 填充commentInfo
    if (comment.rootId === 0) {
        commentInfo.value.root_id = comment.id
    } else {
        commentInfo.value.root_id = comment.rootId;
    }
...
watch(() => route.params.vid, (curr) => {
    commentInfo.value.vid = curr;
    clearCommentList();
    getCommentTree();
});
...
defineExpose({ addComment, clearCommentList, getCommentTree, cancelFocus });
</script>
