                    ...
                    <div class="sub-reply-operation-wrap" :style="{ opacity: operationId === subComment.id ? 1 : 0 }">
                        <div class="reply-operation">
                            ...
                            <ul class="operation-list" :style="{ display: showOption ? '' : 'none' }">
                                <li class="operation-option" v-if="userStore.user?.uid === subComment.user.uid || userStore.user?.uid === upUid"
...
import { emojiText, handleLevel, handleDateTime3 as handleTime } from '@/utils/utils';
import { useUserStore } from '@/stores';
import { getMoreComment as getMoreCommentApi, deleteComment as deleteCommentApi, likeOrDislikeComment } from '@/api';
...
const emit = defineEmits(['sub-reply', 'get-more-comment', 'delSubComment']);

const userStore = useUserStore();

const replyHide = ref(true);
const replyAtUser = ref<any>(null);
const operationId = ref(-1); // 当前鼠标悬停在哪条评论上
const showOption = ref(false); // 点前点击的哪条评论的展开操作栏按钮
const showMoreComment = ref(false);
const currPage = ref(1);   // 当前页码
const totalPage = ref(1);  // 总共页数

// 处理子评论的回复
function handleReply(subComment: any) {
    emit("sub-reply", subComment);
}

async function moreComment() {
    const comment = await getMoreComment();
    emit("get-more-comment", comment);
    showMoreComment.value = true;
}

// 展开更多子评论
async function getMoreComment() {
    const res = await getMoreCommentApi(props.replies[0].rootId);

    return res;
}

// 删除评论前的最后通牒
function beforeDelete(comment: any) {
...
async function likeOrDislike(id: number, like: boolean, set: boolean) {
    ...
    let index = props.replies.findIndex((comment: any) => comment.id === id)
    if (index === -1) {
        ElMessage.error("该评论不存在");
        return;
    }
    let comment = props.replies[index];
...
watch(() => props.replies, (curr) => {
    totalPage.value = Math.ceil(curr.length / 10);
})
</script>
