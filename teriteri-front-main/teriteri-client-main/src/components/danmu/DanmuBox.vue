                        ...
                        <li class="dm-item" v-for="(item, index) in dmList" :key="index">
                            <div class="dm-info-row" @dblclick="handleDblClick(item.timePoint)">
                                ...
                                <span class="dm-info-delete-btn"
                                    v-if="userStore.user?.uid && (authorId === userStore.user?.uid || item.uid === userStore.user?.uid)"
...
<script lang="ts">
export default { name: 'DanmuBox' };
</script>

<script setup lang="ts">
import { ref, watch } from 'vue';
import { ElMessage, ElMessageBox } from 'element-plus';
import { handleTime, handleDateTime2 } from '@/utils/utils';
import { useVideoStore, useUserStore } from '@/stores';
import { deleteDanmu as deleteDanmuApi } from '@/api';
...
const sortType = ref(0);    // 排序方式 0默认日期升序 1时间升序 2时间降序 3内容升序 4内容降序 5日期升序 6日期降序
const dmList = ref<any[]>([]);     // 弹幕列表

// 格式化时间点
function handleTimePoint(time: number) {
    return handleTime(time);
}

// 格式化日期
function handleDateTime(time: number | string | Date) {
...
// 改变排序方式
function resort(type: number) {
    sortType.value = type;
    switch (type) {
        case 1:
            // 时间升序
            dmList.value.sort((a, b) => a.timePoint - b.timePoint);
            break;
        case 2:
            // 时间降序
            dmList.value.sort((a, b) => b.timePoint - a.timePoint);
            break;
        case 3:
            // 内容升序
        ...
        case 5:
            // 日期升序
            dmList.value.sort((a, b) => new Date(a.createDate).getTime() - new Date(b.createDate).getTime());
            break;
        case 6:
            // 日期降序
            dmList.value.sort((a, b) => new Date(b.createDate).getTime() - new Date(a.createDate).getTime());
            break;
        default:
            // 日期升序
            dmList.value.sort((a, b) => new Date(a.createDate).getTime() - new Date(b.createDate).getTime());
...
// 更新弹幕列表
function updateDanmuList(dms: any[]) {
    dmList.value = dms;
    resort(sortType.value);
}

// 双击弹幕跳转播放
function handleDblClick(time: number) {
    emit("jump", time > 0.5 ? time - 0.5 : 0);
}
...
async function deleteDanmu(id: number) {
    const res = await deleteDanmuApi(id);
    if (!res) return;
    videoStore.danmuList = videoStore.danmuList.filter(item => item.id !== id);
    ElMessage.info("删除成功");
}
...
// 深度监听store中弹幕列表的变化，因为对于数组的push操作并不是总能监听到
watch(() => videoStore.danmuList, (curr) => {
    updateDanmuList(curr.slice());
}, { deep: true });
</script>
