<template>
    <div class="header-bar" :class="isFixHeaderBar ? 'slide-down' : ''">
        ...
        <div class="right-entry">
            <!-- 未登录状态 -->
            <div class="header-avatar-wrap" v-if="!userStore.isLogin">
            ...
            <div class="v-popover-wrap">
                <VPopover pop-style="padding-top: 17px;">
                    <template #reference>
                        ...
                        <div class="right-entry--outside" @click="userStore.isLogin ? openNewPage('/message') : dialogVisible = true;">
                            <i class="iconfont icon-xinfeng"></i>
                            <span>消息</span>
                        </div>
                    </template>
                    <template #content>
                        <div class="message-entry-popover" v-if="userStore.isLogin">
                            <div class="message-inner-list">
                                <div class="message-inner-list__item" @click="openNewPage('/message/reply')">
                                    回复我的
                                    <span class="notify-number" v-if="msgStore.msgUnread[0] > 0">
                                        {{ msgStore.msgUnread[0] <= 99 ? msgStore.msgUnread[0] : '99+' }}
                                    </span>
                                </div>
                                <div class="message-inner-list__item" @click="openNewPage('/message/at')">
                                    @ 我的
                                    <span class="notify-number" v-if="msgStore.msgUnread[1] > 0">
                                        {{ msgStore.msgUnread[1] <= 99 ? msgStore.msgUnread[1] : '99+' }}
                                    </span>
                                </div>
                                <div class="message-inner-list__item" @click="openNewPage('/message/love')">
                                    收到的赞
                                    <span class="notify-number" v-if="msgStore.msgUnread[2] > 0">
                                        {{ msgStore.msgUnread[2] <= 99 ? msgStore.msgUnread[2] : '99+' }}
                                    </span>
                                </div>
                                <div class="message-inner-list__item" @click="openNewPage('/message/system')">
                                    系统消息
                                    <span class="notify-number" v-if="msgStore.msgUnread[3] > 0">
                                        {{ msgStore.msgUnread[3] <= 99 ? msgStore.msgUnread[3] : '99+' }}
                                    </span>
                                </div>
                                <div class="message-inner-list__item" @click="openNewPage('/message/whisper')">
                                    我的消息
                                    <span class="notify-number" v-if="msgStore.msgUnread[4] > 0">
                                        {{ msgStore.msgUnread[4] <= 99 ? msgStore.msgUnread[4] : '99+' }}
                                    </span>
                                </div>
                            </div>
                        </div>
            ...
            <div class="v-popover-wrap">
                <VPopover pop-style="padding-top: 17px;">
                    <template #reference>
                        <div class="right-entry--outside" @click="userStore.isLogin ? noPage() : dialogVisible = true;">
                            <i class="iconfont icon-fengche"></i>
                            <span>动态</span>
                        </div>
                    </template>
                    <template #content>
                        <div style="height: 557.3px; width: 371.6px;" v-if="userStore.isLogin">
            ...
            <div class="v-popover-wrap">
                <VPopover :pop-style="userStore.isLogin ? 'padding-top: 17px; margin-left: -100px;' : 'padding-top: 17px;'">
                    <template #reference>
                        <div class="right-entry--outside" @click="userStore.isLogin ? noPage() : dialogVisible = true;">
                    ...
                    <template #content>
                        <div style="height: 556.6px; width: 521.6px;" v-if="userStore.isLogin">
            ...
            <div class="v-popover-wrap">
                <VPopover :pop-style="userStore.isLogin ? 'padding-top: 17px; margin-left: -50px;' : 'padding-top: 17px;'">
                    <template #reference>
                        <div class="right-entry--outside" @click="userStore.isLogin ? noPage() : dialogVisible = true;">
                    ...
                    <template #content>
                        <div style="height: 556.6px; width: 371.6px;" v-if="userStore.isLogin">
            ...
            <div 
                ...
                @click="userStore.isLogin ? openNewPage('/platform') : dialogVisible = true;"
            >
            ...
            <div
                ...
                @click="userStore.isLogin ? openNewPage('/platform/upload') : dialogVisible = true;"
            >
...
<script setup lang="ts">
    ...
    import { handleNum, handleLevel, highlightKeyword as highlightKeywordUtil } from '@/utils/utils';
    import { useUserStore, useMessageStore, useAppStore } from '@/stores';
    import { getMatchingWord as getMatchingWordApi } from '@/api';

    const props = withDefaults(defineProps<{
        /** 是否是固钉导航栏 */
        isFixHeaderBar?: boolean,
        /** 是否显示搜索输入框 */
    ...
    let inTimer: ReturnType<typeof setTimeout> | undefined;  // 节流计时器
    ...
    // 气泡框元素
    const searchPop = ref<HTMLElement | null>(null);

    // 热搜列表（后端实际返回 {content, type} 结构）
    const trendings = computed<any[]>(() => appStore.trendings);
    // 当前登录用户（未登录时兜底为空对象，与原 Vuex state.user 初始值一致）
    const user = computed<any>(() => userStore.user ?? {});
    // 计算消息未读数
    const msgUnread = computed(() => {
        let count = 0;
        for (let i = 0; i < 5; i++) {
            count += msgStore.msgUnread[i];
        }
        return count;
    });
    ...
    async function getMatchingWord() {
        ...
        const keyword = encodeURIComponent(searchInput.value); // 对特殊字符进行编译
    ...
    function handleOutsideClick(event: MouseEvent) {
        const formElement = searchForm.value!; // 输入框元素
        const popoverElement = searchPop.value!; // 气泡框元素
        if (
            !formElement.contains(event.target as Node)
    ...
    // 打开新标签页
    function openNewPage(route: string) {
        window.open(router.resolve(route).href, '_blank');
    }

    // 高亮关键词
    function highlightKeyword(text: string) {
        return highlightKeywordUtil(searchInput.value, text);
    }
    ...
    watch(() => userStore.openLogin, (curr) => {
        if (curr) {
