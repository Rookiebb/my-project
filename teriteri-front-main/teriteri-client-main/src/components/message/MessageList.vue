...
import { handleDateTime, emojiText } from '@/utils/utils';
import { useUserStore, useChatStore } from '@/stores';
import {
    getMoreChatDetails as getMoreChatDetailsApi,
    deleteChatDetailed as deleteChatDetailedApi,
} from '@/api';
...
const error = ref(false);       // 加载更多时出错了
const isAtBottom = ref(true);   // 是否正在底部
const msgId = ref(-1);          // 选中消息的ID
const menuLeft = ref(0);        // 右键菜单左偏移量
const menuTop = ref(0);         // 右键菜单上偏移量

// 请求更多聊天记录
async function getMoreDetails() {
    if (!chat.value.detail.more || loading.value) return;
    loading.value = true;
    const res = await getMoreChatDetailsApi({
        uid: props.mid,
        offset: chat.value.detail.list.length
    });
    // 记录DOM更新前滚动条的位置
    const scrollContainer = document.getElementById('message-list')!;
    const lastChild = scrollContainer.lastElementChild as HTMLElement;
    const lastTop = scrollContainer.scrollTop;
    const lastHeight = lastChild.clientHeight;
    if (res && res.data) {
        const chatItem = chatStore.chatList.find(item => item.user.uid === props.mid);
...
// 判断两个时间是否相差超过4分钟
function timeDiff(small: number | string, big: number | string) {
...
async function handleScroll() {
    const scrollContainer = document.getElementById('message-list')!;
    // 判断是否触底
    const atBottom = scrollContainer.scrollHeight - scrollContainer.scrollTop <= scrollContainer.clientHeight + 10;
    // 判断是否距离顶部小于等于 80 像素
...
// 等元素渲染完再滚
function goBottom(behavior: ScrollBehavior) {
    nextTick(() => {
        const scrollContainer = document.getElementById('message-list')!;
        const lastChild = scrollContainer.lastElementChild as HTMLElement;
        scrollContainer.scrollTo({
...
// 右键消息菜单
function handleContextMenu(id: number, e: MouseEvent) {
    e.preventDefault();
...
function withdraw() {
    ...
    chatStore.ws?.send(JSON.stringify(context));
    msgId.value = -1;
}

// 删除消息
async function deleteMsg() {
    const res = await deleteChatDetailedApi(msgId.value);
    if (res.code && res.code === 200) {
        const chatItem = chatStore.chatList.find(item => item.user.uid === props.mid);
        const index = chatItem!.detail.list.findIndex((item: any) => item.id);
        if (index !== -1) {
...
watch(() => props.mid, (curr) => {
    const chatItem = chatStore.chatList.find(item => item.user.uid === curr);
    // 复制一份，防止指向同一个地址
    chat.value = JSON.parse(JSON.stringify(chatItem));
    goBottom("auto");
});

// 深度监听chatStore中聊天列表的变化
watch(() => chatStore.chatList, (curr) => {
    nextTick(() => {
        const currChat = curr.find(item => item.user.uid === props.mid);
        // 复制粘贴新的chat
        chat.value = JSON.parse(JSON.stringify(currChat));
        if (isAtBottom.value) {
