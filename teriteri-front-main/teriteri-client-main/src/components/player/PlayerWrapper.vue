...
<script lang="ts">
export default { name: 'PlayerWrapper' };
</script>
...
import { getVideoAttitude, addPlayForVisitor } from '@/api';
...
const emit = defineEmits<{
    ...
    (e: 'sendDm', dm: any): void
    (e: 'next'): void
    (e: 'update:jumpTimePoint', value: number): void
}>();

let hideCtrlTimer: any;  // 隐藏控制器的计时器
let clickTimer: any; // 单击播放/暂停事件的防抖计时器
let menuShowTimer: any[] = [null, null, null, null, null, null, null]; // 每个按钮的提示、菜单等显示延时计时器
let menuHideTimer: any[] = [null, null, null, null, null, null, null]; // 每个按钮的提示、菜单等隐藏延时计时器
let speedInterval: any;  // 更新缓冲网速的定时任务

const videoPlayer = ref<HTMLVideoElement | null>(null);

const isMounted = ref(false);   // 做个三秒挂载延迟，防止increasePlay函数重复触发
const wrapSize = ref({
    width: 704,
    height: 442,
});
const videoWidth = ref(704);
const videoHeight = ref(396);
const danmuOpen = ref(true);    // 是否开启弹幕
const dmOpacityOrder = ref(false);  // 更新弹幕不透明度slider的命令
const dmList = ref<any[]>([]);     // 弹幕列表
const lastTimePoint = ref<any>(0);   // 上一个播放时间点，用于筛选将展示的弹幕
const dmIndex = ref(-1);     // 当前时间点之前的最后一个弹幕的指针，播放更新的时候这个弹幕及之前的都不会展示（使用索引可以提升筛选效率，前提是根据时间点排序的弹幕列表）
const dmSetting = ref({
    opacity: 100,     // 弹幕透明度
    dmSpeed: 2,   // 弹幕滚动速度，1慢速75px/s 2正常150px/s 4快速300px/s
});
const rollRow = ref<number[]>(new Array(12).fill(-1));    // 滚动模式下用于记录每一行上一个弹幕的全呈现时间
const topRow = ref<number[]>(new Array(12).fill(-1));    // 顶部模式下用于记录每一行上一个弹幕的结束时间
const bottomRow = ref<number[]>(new Array(12).fill(-1));    // 底部模式下用于记录每一行上一个弹幕的结束时间
const input = ref('');  // 要发送的弹幕内容
const canPlay = ref(false); // 视频可以播放了
const screenType = ref('normal');   // 屏幕类型，normal 正常模式 web 网页全屏 full 屏幕全屏
const ctrlHidden = ref(true);   // 控制器是否隐藏
const pause = ref(false);   // 视频是否暂停中
const buff = ref(false);    // 视频是否正在缓冲
const firstChangeScreen = ref(true);
const currentPer = ref(0);  // 进度条百分比 0~1
const currentTime = ref(0); // 当前播放时间
const bufferPer = ref(0);   // 缓冲百分比 0~1
const bufferSpeed = ref<string | number>(0.00); // 缓冲网速 KB/s
const videoSize = ref(0);   // 视频字节大小
const lastByte = ref(0);    // 上次缓冲的字节
const isMuted = ref(false); // 是否静音
const volumeOrder = ref(false); // 更新音量slider的命令
const volume = ref(35);     // 音量
const showDmInput = ref(false); // 是否显示全屏状态下的弹幕输入框
const playbackrate = ref('倍速');   // 倍速文本
const dmTips = ref(false);  // 弹幕按钮快捷提示是否显示
const fullTips = ref(false);  // 全屏按钮快捷提示是否显示
const setting = ref({
    loop: false,  // 是否开启洗脑循环
...
function changeWindowSize() {
    ...
    let rightWidth: number, bottomHeight: number;   // 整体布局的right-container宽度和底部的发送弹幕栏的高度
    if (windowWidth < 1681) {
        rightWidth = 380;
        bottomHeight = 46;
    } else {
    ...
    videoHeight.value = heigth - bottomHeight;
    emit('resize', wrapSize.value);
}
...
function handleKeyboard(event: KeyboardEvent) {
    ...
    // →键 快进5秒
        ...
        // 初始化弹幕索引
        removeAllDanmu();
        lastTimePoint.value = videoPlayer.value!.currentTime;
        dmIndex.value = -1;
...
function playVideo() {
    ...
    speedInterval = setInterval(updateBufferSpeed, 1000);  // 启动定时更新缓冲网速任务
}
...
function changeSetting(i: number) {
    if (i === 1) {
        if (setting.value.loop) {
            setting.value.loop = false;
        } else {
            setting.value.loop = true;
        }
    } else if (i === 2) {
        if (setting.value.autoplay) {
            setting.value.autoplay = false;
        } else {
            setting.value.autoplay = true;
        }
    } else if (i === 3) {
        if (setting.value.autonext) {
            setting.value.autonext = false;
        } else {
            setting.value.autonext = true;
        }
        emit("update:autonext", setting.value.autonext);
    }
    const settingStr = JSON.stringify(setting.value);
    localStorage.setItem('playerSetting', settingStr);
}
...
// 改变音量
function changeVolume(per: number) {
    let vol = Math.floor(per * 100);
    volume.value = vol;
...
// 更新弹幕透明度
function changeDmOpacity(per: number) {
    let opacity = Math.floor(per * 100);
    dmSetting.value.opacity = opacity;
}
...
// 加载展示弹幕
function displayDanmus(currTimePoint: number) {
    // console.log(currTimePoint, lastTimePoint.value);
    const dmWrap = document.querySelector('.player-row-dm-wrap')!;
    const screen = document.querySelector('.player-video-area') as HTMLElement;
    // 选出当前时间需要展示的弹幕
    // 一定要 lastTimePoint < timePoint <= currTimePoint
    // 发送弹幕时一定要是 currentTime 不能是videoPlayer.value.currentTime
    ...
    while(dmIndex.value + 1 < dmList.value.length && dmList.value[dmIndex.value + 1].timePoint <= currTimePoint) {
        ...
        if (dm.timePoint > lastTimePoint.value) {
            ...
            danmuElement.style.setProperty('--opacity', `${dmSetting.value.opacity / 100}`);
            danmuElement.style.setProperty('--fontSize', `${dm.fontsize}px`);
            danmuElement.style.setProperty('--color', `${dm.color}`);
            // 标识自己发的弹幕
            if (userStore.user?.uid && dm.uid === userStore.user?.uid) {
            ...
            if (dm.mode === 1) {
                ...
                danmuElement.style.setProperty('--duration', `${distance / (dmSetting.value.dmSpeed * 75)}s`);
                // 计算弹幕位置
                const rowOutTime = (contentWidth + 10) / (dmSetting.value.dmSpeed * 75) + currTimePoint; // 当前弹幕全部呈现时的时间点
                let i = 0, full = false, min = -1, minIndex = 0;
                while (i < 12) {
                    if (rollRow.value[i] <= currTimePoint) {
                ...
                const duration = dmSetting.value.dmSpeed === 1 ? 6 : dmSetting.value.dmSpeed === 2 ? 4 : 2;
...
function displaySentDanmus() {
    ...
    danmuElement.style.setProperty('--opacity', `${dmSetting.value.opacity / 100}`);
    danmuElement.style.setProperty('--fontSize', `${dmStyle.value.fontsize}px`);
    danmuElement.style.setProperty('--color', `${dmStyle.value.color}`);
    danmuElement.style.setProperty('border', '2px solid #FFFFFF');
    dmWrap.appendChild(danmuElement);
    if (dmStyle.value.mode === 1) {
        // 滚动模式
        danmuElement.classList.add('roll');
        const contentWidth = danmuElement.offsetWidth;    // 内容长度
        const distance = screen.offsetWidth + contentWidth;    // 总位移距离
        ...
        danmuElement.style.setProperty('--duration', `${distance / (dmSetting.value.dmSpeed * 75)}s`);
        // 计算弹幕位置
        const rowOutTime = (contentWidth + 10) / (dmSetting.value.dmSpeed * 75) + currentTime.value; // 当前弹幕全部呈现时的时间点
        let i = 0, full = false, min = -1, minIndex = 0;
        while (i < 12) {
...
function sendDm() {
    ...
    const dm = {
        content: input.value,
        fontsize: dmStyle.value.fontsize,
        mode: dmStyle.value.mode,
        color: dmStyle.value.color,
        timePoint: currentTime.value     // 不能是videoPlayer.value.currentTime
    }
    emit("sendDm", dm);
    displaySentDanmus();
    input.value = '';
...
// 外部控制的时间跳转（双击弹幕）
function jumpTimePointAndPlay(time: number) {
    // 计算进度条的位置
    currentPer.value = time / props.duration;
...
// 拖动进度条的回调
function changeCurrentPer(curr: number) {
    currentPer.value = curr;
    // 计算当前时间
    let time = curr * props.duration;
    currentTime.value = time;
    videoPlayer.value!.currentTime = time;
    initDanmuIndex(time);
}
...
// 初始化弹幕索引
function initDanmuIndex(time?: any) {
    removeAllDanmu();
...
function updateBufferingBar() {
    ...
    if (buffered.length > 0) {
        // 计算已缓冲的百分比并更新缓冲进度条
        bufferPer.value = buffered.end(buffered.length - 1) / props.duration;
    }
}

// 计算缓冲网速
function updateBufferSpeed() {
    const buffered = videoPlayer.value!.buffered;
    if (buffered && buffered.length > 0) {
        let currByte = buffered.end(buffered.length - 1) / props.duration * videoSize.value;
        let differ = currByte - lastByte.value;    // 在1s内加载了这么多字节
        bufferSpeed.value = (differ / 1000).toFixed(2);
        lastByte.value = currByte;
    }
}

// 调整倍速
function changePlaybackRate(num: number) {
    if (num === 1.0) {
...
// 鼠标悬停按钮时打开菜单
function enterBtn(name: string, index: number) {
...
// 鼠标移出按钮时关闭菜单
function leaveBtn(name: string, index: number) {
...
if (localStorage.getItem("dmSetting")) {
    dmSetting.value = JSON.parse(localStorage.getItem("dmSetting")!);
}
// 同步播放设置
if (localStorage.getItem("playerSetting")) {
    setting.value = JSON.parse(localStorage.getItem("playerSetting")!);
}
// 判断是否允许自动播放
if (!setting.value.autoplay) {
    pause.value = true;
...
if (localStorage.getItem("dmStyle")) {
    dmStyle.value = JSON.parse(localStorage.getItem("dmStyle")!);
}
// 获取弹幕列表
// updateDanmuList(videoStore.danmuList);
...
onMounted(() => {
    ...
    // videoPlayer.value.addEventListener('progress', updateBufferingBar);
    // 默认播放音量不等于volume，所以挂载时更新同步一下音量
    videoPlayer.value!.volume = volume.value / 100;
    setTimeout(() => {
...
watch(() => dmStyle.value.fontsize, () => {
    localStorage.setItem("dmStyle", JSON.stringify(dmStyle.value));
});
watch(() => dmStyle.value.mode, () => {
    localStorage.setItem("dmStyle", JSON.stringify(dmStyle.value));
});
watch(() => dmStyle.value.color, () => {
    localStorage.setItem("dmStyle", JSON.stringify(dmStyle.value));
});
watch(() => dmSetting.value.opacity, () => {
    localStorage.setItem("dmSetting", JSON.stringify(dmSetting.value));
    dmOpacityOrder.value = true;
});
watch(() => dmSetting.value.dmSpeed, () => {
    localStorage.setItem("dmSetting", JSON.stringify(dmSetting.value));
});
...
watch(() => props.jumpTimePoint, (curr) => {
    if (curr >= 0) {
        jumpTimePointAndPlay(curr);
        emit("update:jumpTimePoint", -1);
    }
});
watch(() => props.autonext, (curr) => {
    setting.value.autonext = curr;
    const settingStr = JSON.stringify(setting.value);
    localStorage.setItem('playerSetting', settingStr);
});
// 更换视频时初始化
