...
import { handleTime as handleDuration, handleNum, handleDate } from '@/utils/utils';
import { useAppStore } from '@/stores';
import { getRandomVideos as fetchRandomVideos, getCumulativeVideos as fetchCumulativeVideos } from '@/api';
...
// 频道列表
const channels = computed(() => appStore.channels as any[]);
...
async function getCumulativeVideos() {
    ...
    const res = await fetchCumulativeVideos(ids);
    if (res.data) {
        cumulativeVideos.value.push(...res.data.videos);
...
let handleMouseOut: ((e: MouseEvent) => void) | undefined;
...
async function handleScroll() {
    ...
    // 导航栏
    if (el.scrollTop >= 64 && !isFixHeaderBar.value) {
        isFixHeaderBar.value = true;
    } else if (el.scrollTop < 64 && isFixHeaderBar.value) {
        isFixHeaderBar.value = false;
    }

    // 频道栏
    if (el.scrollTop >= headerHight && !isFixChannel.value) {
        isFixChannel.value = true;
    } else if (el.scrollTop < headerHight && isFixChannel.value) {
...
onMounted(async () => {
    ...
    headerHight = (document.querySelector(".main__layout") as HTMLElement).offsetTop;
...
onBeforeUnmount(() => {
    ...
    if (handleMouseOut) document.removeEventListener('mouseout', handleMouseOut);
