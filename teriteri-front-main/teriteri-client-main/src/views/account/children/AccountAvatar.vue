...
<script lang="ts">
export default { name: "AccountAvatar" };
</script>
...
const imgUrl = ref<string | null | undefined>(null);    // 预览图地址
const wrapWidth = ref(96);  // 预览容器宽   手动设置
const wrapHeight = ref(96); // 预览容器高   手动设置
const prevWidth = ref(96);  // 预览图实际宽 初始值设为和上面一致
const prevHeight = ref(96); // 预览图实际高 初始值设为和上面一致
const prevOffsetX = ref(0); // 预览图水平偏移量
const prevOffsetY = ref(0); // 预览图垂直偏移量
const cutOrder = ref(false);    // 裁剪命令 由false变为true就裁剪
const loading = ref(false); // 正在裁剪并上传

const isCurrAvatar = computed(() => {
    return !imgUrl.value || imgUrl.value === userStore.user?.avatar_url || !userStore.user?.uid;
});

// 初始化
function init() {
    prevWidth.value = wrapWidth.value;
    prevHeight.value = wrapHeight.value;
    prevOffsetX.value = 0;
    prevOffsetY.value = 0;
}
...
// 处理拖拽事件
function handlePicDrop(event: DragEvent) {
...
// 发送裁剪指令
function submit() {
    if (loading.value || isCurrAvatar.value) return;
    loading.value = true;
    cutOrder.value = true;
}

// 将收到的裁剪好的图片发送后端
function send(img: string) {
    fetch(img)
    .then(response => response.blob())
    .then(blob => {
        const avatar = new File([blob], '新头像.jpg', { type: 'image/jpeg' });  // 名字不重要，后端会重命名
        // 发送请求
        updateUserAvatar({ file: avatar })
        .then(res => {
            if (res.code === 200) {
                ElMessage.success("头像更新成功");
                if (userStore.user) {
                    userStore.user.avatar_url = res.data;
                }
            }
        })
    })
...
onMounted(() => {
    if (userStore.user?.avatar_url) {
        imgUrl.value = userStore.user.avatar_url;
    }
});

watch(() => userStore.user, (curr) => {
    imgUrl.value = curr?.avatar_url;
    init();
}, { deep: true });
</script>
