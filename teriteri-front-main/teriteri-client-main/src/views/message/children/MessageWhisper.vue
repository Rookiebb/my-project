        ...
        <div class="im">
            <div class="left">
                ...
                <div class="list-container" id="chat-list">
                    <div class="list">
                        <div class="list-item" :class="{'active': chatStore.chatId === item.user.uid}"
                            @click="changeChat(item.user.uid)"
                            v-for="(item, index) in chatStore.chatList" :key="index"
                        >
                            ...
                            <div class="name-box">
                                ...
                                <div class="last-word" v-if="item.detail.list.length > 0">
                                    {{ item.detail.list[item.detail.list.length - 1].withdraw === 1 ? `${item.detail.list[item.detail.list.length - 1].userId === userStore.user?.uid ? '你' : '对方'}撤回了一条消息` : item.detail.list[item.detail.list.length - 1].content }}
                                </div>
                            </div>
                            <div class="close">
            ...
            <div class="right">
                <router-view v-if="chatStore.chatId > 0"></router-view>
...
<script lang="ts">
export default { name: "MessageWhisper" };
</script>
...
import { getRecentChatList, createChat as createChatApi, deleteChat } from '@/api';
...
const more = ref(true);     // 是否还有更多聊天
const loading = ref(false);     // 是否正在加载更多聊天
...
async function getChatList() {
    ...
    loading.value = true;
    const res = await getRecentChatList(chatStore.chatList.length);
    if (res.data) {
        more.value = res.data.more;
        chatStore.chatList.push(...res.data.list);
    }
    nextTick(() => {
        loading.value = false;
    });
...
async function createChat() {
    const res = await createChatApi(Number(route.params.mid));
    if (res.code === 200) {
        if (res.data) {
            // 新创建
            chatStore.chatList.unshift(res.data);
        }
        chatStore.chatId = Number(route.params.mid);
    } else {
        chatStore.chatId = -1;
    }
}
...
async function closeChat(uid: number) {
    let i = chatStore.chatList.findIndex(item => item.user.uid === uid);
    chatStore.chatList.splice(i, 1);    // 移除这个元素
    if (Number(route.params.mid) === uid) {
        // 如果移除的是当前聊天 就打开第一个聊天
        if (chatStore.chatList.length > 0) {
            changeChat(chatStore.chatList[0].chat.userId);
        } else {
            chatStore.chatId = -1;
        }
    }
...
///////// 事件 /////////
// 切换聊天
function changeChat(mid: number) {
