...
<script lang="ts">
export default { name: 'WhisperDialog' };
</script>
...
import { updateWhisperOnline, updateWhisperOutline } from '@/api';
...
const mid = ref(0);     // 当前聊天的对象的uid
// 当前聊天的对象的用户信息 这里只列出相关列用于页面挂载，挂载后会整个user更新
const user = ref<any>({
    uid: null,
    nickname: null,
});
const myId = ref<any>(null);    // 暂存自己的uid 以便登录过期时可以发送离线请求
const emojiBoxShow = ref(false);    // 表情框显示的双向绑定
const input = ref("");     // 保存输入的内容
const emojiMap = EmojiJson;    // 表情列表
const isComposite = ref(false);     // 是否正在输入中文
const rangeOfEditor = ref<Range | null>(null);    // 保存光标位置
...
// 更新对方用户 以及缓存自己的id
function updateUser() {
    let i = chatStore.chatList.findIndex(item => item.user.uid === mid.value);
    user.value = chatStore.chatList[i].user;
    myId.value = chatStore.chatList[i].chat.anotherId;
}
...
// 发送消息
function sendMsg() {
    if (user.value.uid === userStore.user?.uid) {
    ...
    chatStore.ws!.send(JSON.stringify(msg));
    // 清空文本
...
function handleOutsideClick(event: MouseEvent) {
    const emojiBox = emoji.value!;
    if (!emojiBox.contains(event.target as Node)) {
...
function handleInput() {
    ...
    for (let i = 0; i < emojiMap.length; i++) {
        const emojiCode = emojiMap[i].name;
        const emojiUrl = emojiMap[i].url;
...
function insertEmoji(emoji: any) {
    ...
    const range = rangeOfEditor.value;
    // 如果是选择模式
    if (!range.collapsed)  {
        if (
            ...
            // 选中同一段文本的全部 删除该段文本元素
            editor.value!.removeChild(range.endContainer);
        }
        else if (
            range.startContainer !== range.endContainer &&
            range.startContainer.nodeName === "#text" &&
            ...
            const i = childNodesArray.indexOf(range.startContainer);
            const j = childNodesArray.indexOf(range.endContainer);
            for (let k = i; k <= j; k ++) {
    ...
    if (range.endContainer.nodeName === "#text") {
        const childNodesArray = Array.from(editor.value!.childNodes);
        const i = childNodesArray.indexOf(range.endContainer);

        if ((range.endContainer as Text).length === range.endOffset) {
            // 光标在文本末尾 设置光标到编辑器下文本域后边
            let newRange = new Range();
            newRange.selectNodeContents(editor.value!);
            newRange.setStart(editor.value!, i + 1);
            newRange.setEnd(editor.value!, i + 1);
            newRange.collapse(true);
            rangeOfEditor.value = newRange;
        } else if (range.endOffset === 0) {
            // 光标在文本开头 设置光标到编辑器下文本域前边
            let newRange = new Range();
            newRange.selectNodeContents(editor.value!);
            newRange.setStart(editor.value!, i);
            newRange.setEnd(editor.value!, i);
            newRange.collapse(true);
            rangeOfEditor.value = newRange;
        }
    }
    ...
    rangeOfEditor.value!.collapse(false);     // 这个我也不知道为什么要设置false，但是true会出bug
    // 将焦点设置回输入框
    editor.value!.focus();
