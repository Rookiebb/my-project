<template>
    <div class="platform-view">
        ...
        <div class="platform-header">
            <div class="header-content">
                <div class="left-block">
                    <div class="logo-container" @click="router.push('/platform/home')">
                ...
                <div class="right-block">
                    ...
                    <VPopover pop-style="padding-top: 10px;">
                        <template #reference>
                            <div class="message" @click="userStore.isLogin ? openNewPage('/message') : noLogin()">
                            ...
                            <div class="message-entry-popover" v-if="userStore.isLogin">
                                <div class="message-inner-list">
                                    <div class="message-inner-list__item" @click="openNewPage('/message/reply')">
                                        回复我的
                                        <span class="notify-number" v-if="messageStore.msgUnread[0] > 0">
                                            {{ messageStore.msgUnread[0] <= 99 ? messageStore.msgUnread[0] : '99+' }}
                                        </span>
                                    </div>
                                    <div class="message-inner-list__item" @click="openNewPage('/message/at')">
                                        @ 我的
                                        <span class="notify-number" v-if="messageStore.msgUnread[1] > 0">
                                            {{ messageStore.msgUnread[1] <= 99 ? messageStore.msgUnread[1] : '99+' }}
                                        </span>
                                    </div>
                                    <div class="message-inner-list__item" @click="openNewPage('/message/love')">
                                        收到的赞
                                        <span class="notify-number" v-if="messageStore.msgUnread[2] > 0">
                                            {{ messageStore.msgUnread[2] <= 99 ? messageStore.msgUnread[2] : '99+' }}
                                        </span>
                                    </div>
                                    <div class="message-inner-list__item" @click="openNewPage('/message/system')">
                                        系统消息
                                        <span class="notify-number" v-if="messageStore.msgUnread[3] > 0">
                                            {{ messageStore.msgUnread[3] <= 99 ? messageStore.msgUnread[3] : '99+' }}
                                        </span>
                                    </div>
                                    <div class="message-inner-list__item" @click="openNewPage('/message/whisper')">
                                        我的消息
                                        <span class="notify-number" v-if="messageStore.msgUnread[4] > 0">
                                            {{ messageStore.msgUnread[4] <= 99 ? messageStore.msgUnread[4] : '99+' }}
                                        </span>
                                    </div>
                                </div>
                            </div>
        ...
        <div class="platform-nav">
            <div class="nav-upload-btn" @click="router.push('/platform/upload')">
        ...
        <!-- 主体 -->
        <div class="platform-main" v-if="userStore.isLogin">
...
<script lang="ts">
export default { name: "PlatformView" };
</script>
...
const active = ref("");
const path = ["/platform/home", "/platform/upload-manager/manuscript", "/platform/upload-manager/appeal", "/platform/data-up", "/platform/comment", "/platform/danmu"];
...
const msgUnread = computed(() => {
    let count = 0;
    for (let i = 0; i < 5; i++) {
        count += messageStore.msgUnread[i];
    }
    return count;
});
