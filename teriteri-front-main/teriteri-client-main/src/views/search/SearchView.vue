...
import { addSearchWord as addSearchWordApi, getSearchCount } from '@/api';
...
const routerIndex = computed(() => {
    return routerList.findIndex(item => route.path.startsWith(item.path));
});

// 添加搜索词条
function addSearchWord() {
    addSearchWordApi(props.keyword);
    getCount();
}
...
async function getCount() {
    ...
    const res = await getSearchCount(keyword);
    searchStore.setMatchingCount(res.data);
}
