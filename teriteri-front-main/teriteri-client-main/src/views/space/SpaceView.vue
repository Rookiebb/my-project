            ...
            <div class="h">
                <div class="wrapper">
                    ...
                    <div class="h-inner" :style="`background-image: url('${user.bg_url}');`">
                        ...
                        <div class="h-user">
                            <div class="h-info">
                                <div class="avatar-container">
                                    ...
                                    <a v-if="user.uid === currentUid" href="/account/avatar" target="_blank" class="avatar-cover">更换头像</a>
                                </div>
                                <div class="h-basic">
                                    <div class="basic-top">
                        ...
                        <div class="h-action" v-if="user.uid !== currentUid">
                        ...
                        <div title="更换皮肤" class="space-theme-trigger icon" v-if="user.uid === currentUid"
                    ...
                    <!-- 更换主题面板 -->
                    <div class="space-theme" :class="{'theme-show': themeShow}" v-if="user.uid === currentUid">
                    ...
                    <div class="n-inner">
                        ...
                        <div class="n-tab-links" @mouseleave="hoverIdx = -1">
                            ...
                            <div class="n-btn n-favlist" :class="{'active': navIdx === 4}" @click="clickNavItem('/favlist')" @mouseenter="hoverIdx = 4">
                                ...
                                <span class="n-num">{{ userFavList.length }}</span>
                            </div>
                            <div class="n-btn n-setting" v-if="currentUid === user.uid" :class="{'active': navIdx === 5}" @click="clickNavItem('/setting')" @mouseenter="hoverIdx = 5">
...
import { getUserInfo as getUserInfoApi, getUserWorksCount as getUserWorksCountApi, getFavoriteListForUser, getFavoriteListForVisitor } from '@/api';
import { TOKEN_KEY } from '@/constants';
import { handleNum, handleLevel } from '@/utils/utils';
...
const worksCount = ref(0);   // 用户投稿数
const themePosX = ref(0);    // 右上角的样式动画效果
let timer: ReturnType<typeof setInterval> | null = null;
const themeShow = ref(false);    // 是否展开更换头图的抽屉
const hoverIdx = ref(-1);    // 悬停导航栏
const mounted = ref(false);
const userFavList = ref<any[]>([]);    // 用户的收藏夹列表（共享给子路由组件）

provide('userFavList', userFavList);

// 当前登录用户的 uid（未登录时为 undefined）
...
const barWidth = computed(() => {
    ...
    let target = '.n-index';
    if (hoverIdx.value >= 0) {
        target = list[hoverIdx.value];
    } else if (navIdx.value < 6) {
        target = list[navIdx.value];
    }
    if (!mounted.value) return 0;
    return document.querySelector(target)?.clientWidth ?? 0;
});
...
const barLeft = computed(() => {
    ...
    const wrapperEl = document.querySelector(".n-wrapper");
    if (!targetEl || !wrapperEl) return 0;
    return targetEl.getBoundingClientRect().left - wrapperEl.getBoundingClientRect().left;
});
...
// 获取用户的收藏夹列表
async function getUserFavList(uid: number) {
    let res;
    if (!localStorage.getItem(TOKEN_KEY)) {
        res = await getFavoriteListForVisitor(uid);
    } else {
        res = await getFavoriteListForUser(uid);
    }
    if (!res) return;
    // 将默认置顶
    ...
    userFavList.value = list;
}

// 获取用户投稿数
async function getUserWorksCount(uid: number) {
    const res = await getUserWorksCountApi(uid);
    if (!res) return;
    worksCount.value = res.data;
}
...
// 打开新标签页
function openNewPage(path: string) {
    window.open(router.resolve(path).href, '_blank');
}

// 开始计时器
function startTimer(isIncrease: boolean) {
    // 清除之前的计时器
    if (timer) clearInterval(timer);
    // 设置计时器，每 0.02 秒执行一次
    timer = setInterval(() => {
        // 根据移入移出状态，增加或减少 themePosX
        if (isIncrease && themePosX.value < 522) {
            themePosX.value += 58;
        } else if (!isIncrease && themePosX.value > 0) {
            themePosX.value -= 58;
        } else {
...
// 点击导航栏跳转页面
function clickNavItem(path: string) {
    const absPath = `/space/${route.params.uid}${path}`;
    if (route.path === absPath) return;
    router.push(absPath);
}
...
async function init() {
    if (route.path === "/space" || route.path === "/space/") {
        ...
        if (token) {
            try {
                // 解析JWT获取当前用户uid
                const w = token.split('.')[1];
                const info = JSON.parse(atob(w));
                router.push(`/space/${info.sub}`);
            } catch (e) {
                router.push("/");
            }
        } else {
        ...
        if (uid && uid !== 0) {
            await getUserFavList(uid);
            await getUserInfo(uid);
            await getUserWorksCount(uid);
        } else {
            router.push('/notfound');
        }
    }
...
watch(() => route.params.uid, async (curr) => {
    if (!curr) return;
    // 判断路由参数合法性
