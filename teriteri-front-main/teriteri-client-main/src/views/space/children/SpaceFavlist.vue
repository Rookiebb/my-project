        ...
        <div class="fav-sidenav">
            <div class="nav-container">
                ...
                <div class="fav-list-container ps">
                    ...
                    <ul class="fav-list">
                        <li class="fav-item" :class="{'cur': Number(fid) === item.fid, 'owner': currentUid === uid}" v-for="(item, index) in favList" :key="index">
                ...
                <ul class="fav-video-list clearfix">
                    <li class="small-item" v-for="(item, index) in favVideos" :key="index">
                        ...
                        <VPopover class="video-edit" popStyle="position: fixed; padding-top: 10px; z-index: 1000; top: auto; left: auto; transform: translate3d(-75%,0,0);">
                            ...
                            <template #content>
                                <ul class="be-dropdown-menu" v-if="currentUid === currFav.uid">
...
import { getUserCollectVideos, cancelCollect as cancelCollectApi } from '@/api';
import { handleTime, handleNum, handleDate } from '@/utils/utils';
...
const isFavnavOpen = ref(true);
const rule = ref(1);    // 排序规则 1 最近收藏 2 最多播放 3 最新投稿
const page = ref(1);    // 当前分页
const favVideos = ref<any[]>([]);  // 当前收藏夹的视频列表
...
// 用户的收藏夹列表（由父组件 SpaceView 获取并注入）
const userFavList = inject<Ref<any[]>>('userFavList', ref<any[]>([]));

// 收藏夹列表
const favList = computed(() => userFavList.value);

// 当前收藏夹
const currFav = computed(() => {
    const fav = favList.value.find(item => item.fid === Number(props.fid));
    if (!fav || (fav.uid !== currentUid.value && fav.visible === 0)) {
        return null;
    } else {
        return fav;
    }
...
async function getFavVideos() {
    favVideos.value = [];
    if (!currFav.value) return;
    const res = await getUserCollectVideos({
        fid: currFav.value.fid,
        rule: rule.value,
        page: page.value,
        quantity: 20
    });
...
// 取消收藏
async function cancelCollect(info: any) {
    const res = await cancelCollectApi({ vid: info.vid, fid: info.fid });
    if (!res || res.code !== 200) return;
    favVideos.value = favVideos.value.filter(item => item.video.vid !== info.vid);
}

function changeFav(fid: number) {
    router.push(`/space/${uid.value}/favlist?fid=${fid}`);
}
...
onMounted(() => {
    if (!props.fid) {
        const fav = favList.value.find(item => item.type === 1);
        router.push(`/space/${uid.value}/favlist?fid=${fav.fid}`);
    } else {
        getFavVideos();
    }
});
