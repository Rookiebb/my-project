        ...
        <div class="col-1">
            ...
            <div class="section video">
                <h3 class="section-title">
                    <span class="t" @click="$router.push(`/space/${uid}/video`)">{{ currentUid === uid ? '我' : 'TA' }}的视频</span>
            ...
            <div class="section article">
                <h3 class="section-title">
                    <span class="t" @click="$router.push(`/space/${uid}/article`)">{{ currentUid === uid ? '我' : 'TA' }}的专栏</span>
            ...
            <div class="section fav">
                <h3 class="section-title">
                    <span class="t" @click="$router.push(`/space/${uid}/favlist`)">{{ currentUid === uid ? '我' : 'TA' }}的收藏夹</span>
                ...
                <div class="fav-content">
                    <div v-for="(item, index) in favList" :key="index">
                        <div class="fav-item" v-if="item.visible === 1 || item.uid === currentUid">
        ...
        <div class="col-2">
            <div class="section user-info">
                <p class="user-info-title">
                    ...
                    <a href="/account/info" target="_blank" v-if="uid === currentUid">
...
import { getUserWorks, getUserLoveVideos, getFavoriteListForUser, getFavoriteListForVisitor } from '@/api';
...
const rule = ref(1);    // 视频列表排序方式
const videoList = ref<any[]>([]);  // 用户投稿视频列表
const videoCount = ref(0);  // 用户视频投稿数量
const favList = ref<any[]>([]);    // 用户收藏夹列表
const loveVideos = ref<any[]>([]); // 用户最近点赞视频列表
...
async function getFavList() {
    ...
    if (!localStorage.getItem(TOKEN_KEY)) {
        res = await getFavoriteListForVisitor(uid.value);
    } else {
        res = await getFavoriteListForUser(uid.value);
    }
    if (!res) return;
    // 将默认置顶
...
// 获取用户最近点赞的视频
async function getLoveVideos() {
    const res = await getUserLoveVideos({
        uid: uid.value,
        offset: 0,
        quantity: 20
    });
    if (!res) return;
    loveVideos.value = res.data;
}
...
watch(rule, () => {
    ...
    videoList.value.forEach((el, index) => videoList.value[index] = item);
    getVideoList();
});
</script>
